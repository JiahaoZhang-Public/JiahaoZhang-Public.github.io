---
permalink: /
title: "About me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

Hi there! 👋 I’m a senior undergraduate student majoring in Electrical Engineering (Electronic Packaging Technology) at [Huazhong University of Science and Technology (HUST)](https://www.hust.edu.cn/). I recently completed my exchange program at UC Berkeley, where I delved into CS courses. 

🌟 **Exciting News!** 🌟  
On June 11th, I joined the [Yuan Fajie Lab of Representation Learning](https://fajieyuan.github.io/) at Westlake University as a visiting student. My current research interests revolve around protein editing and multimodal protein language models. Our lab focuses on leveraging AI to solve challenging problems for biologists, accelerating the advancement of biological sciences. 

Check out our recent work:
- [SaProt](https://github.com/westlake-repl/SaProt)
- [SaProtHub](https://github.com/westlake-repl/SaProtHub)
- [ProTrek](https://github.com/westlake-repl/ProTrek)

🎉 **Past News** 🎉  
- Our project at Grapedata received the Cloud Computing Application Award in the UCB Data Science Discovery Program, standing out among over 100 teams! You can find more details on my [blog post](../_posts/2024-05-01-Award.md).
- I completed my internship at [Grapedata](https://www.grape-data.com/), where I worked as a data science research intern.
- I also finished my exchange program at UC Berkeley, where I took various CS courses and participated in the Data Science Discovery Program.

My research interests include:
* **Fintech**: Utilizing LLMs to develop textual analysis in asset pricing and portfolio management.
* **Machine Learning and AI**: Exploring the potential of advanced LLMs to process real-time physical signals (which could have future applications in the Internet of Things, IoT) and discovering the possibilities of LLMs as decision-making brains for AI agents.

I’m fortunate to be advised by [Prof. Weijie](http://faculty.hust.edu.cn/weijie1/zh_CN/index.htm) at HUST. Previously, I was advised by [Prof. Chaowei](https://xiaocw11.github.io/) at the University of Wisconsin, Madison, and worked with his doctoral student Fangzhou Wu and [Qingzhao Zhang](https://zqzqz.github.io/) at the University of Michigan.

You can find my CV here: [Jiahao's Curriculum Vitae](../assets/CV.pdf).

⚠️ Note: "Projects" and "CV" sections are still in the process of being updated, and "Publications" and "Blog Posts" are not available yet. I’m working hard on my first paper, so stay tuned!

Feel free to reach out: [Email](mailto:jiahao.zhang.public@gmail.com) / [edu Email](mailto:jiahao.zhang_public@berkeley.edu) / [Github](https://github.com/JiahaoZhang-Public/)

<p align="center"> 
  Visitor count<br>
  <img src="https://profile-counter.glitch.me/JiahaoZhang-Public/count.svg" />
</p>